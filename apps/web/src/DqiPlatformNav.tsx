...
import { themedHref, useDqiTheme } from './ThemeToggle.js';

const products = [
  { id: 'home', name: 'DQI Home', description: 'Platform overview, products and access', href: import.meta.env.VITE_DQI_HOME_URL || 'https://webpage-examples-dqi.onrender.com', accent: '#f8fafc' },
  ...
  { id: 'assess', name: 'DQI Assess', description: 'Free assessment · maturity and readiness', href: import.meta.env.VITE_DQI_ASSESS_URL || 'https://dqi-assess-s7sp.onrender.com/assessment/welcome', accent: '#ffd166' },
  { id: 'enforce', name: 'DQI Enforce', description: 'Free trial · runtime policy controls', href: import.meta.env.VITE_DQI_ENFORCE_URL || 'https://dqi-enforce-demo.onrender.com', accent: '#51e4bb' },
  { id: 'analytics', name: 'DQI Analytics', description: 'Free trial · evidence and audit reporting', href: '/', accent: '#809dff' },
] as const;

export function DqiPlatformNav() {
  const [open, setOpen] = useState(false);
  const { theme } = useDqiTheme();
    ...
    <div className="platform-switcher" ref={menuRef} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
      ...
      {open && (
        <div className="platform-menu" role="menu">
          <header><div><strong>DQI platform</strong><small>One framework. Four connected products.</small></div><span>FREE TRIAL</span></header>
          <div className="platform-products">
            {products.map((product) => {
              const active = product.id === 'analytics';
              return (
                <a key={product.id} role="menuitem" href={themedHref(product.href, theme)} aria-current={active ? 'page' : undefined} className={active ? 'active' : undefined}>
