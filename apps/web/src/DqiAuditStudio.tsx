...
export function DqiAuditStudio() {
  ...
  return <main className="dqi-shell">
    ...
    <details className="guided-builder"><summary>Build a report step by step</summary><p>Pick governed semantic building blocks, then edit the generated sentence however you like.</p><div className="builder-flow"><label><span>1 · Metric</span><select value={guidedMetric} onChange={(event) => setGuidedMetric(event.target.value)}>{guidedMetrics.map((item) => <option key={item}>{item}</option>)}</select></label><label><span>2 · Dimension</span><select value={guidedDimension} onChange={(event) => setGuidedDimension(event.target.value)}>{guidedDimensions.map((item) => <option key={item}>{item}</option>)}</select></label><label><span>3 · Calculation</span><select value={guidedCalculation} onChange={(event) => setGuidedCalculation(event.target.value)}>{guidedCalculations.map((item) => <option key={item.label} value={item.phrase}>{item.label}</option>)}</select></label><label><span>4 · Visual</span><select value={guidedView} onChange={(event) => setGuidedView(event.target.value)}>{guidedViews.map((item) => <option key={item}>{item}</option>)}</select></label><label><span>5 · Window</span><select value={guidedRange} onChange={(event) => setGuidedRange(event.target.value)}><option value="4">4 weeks</option><option value="12">12 weeks</option><option value="26">26 weeks</option></select></label></div><div className="builder-preview"><code>{guidedPrompt}</code><button onClick={() => setPrompt(guidedPrompt)}>Use this prompt →</button></div><small>Calculations are governed formula presets. Qwen may interpret your wording, but only the deterministic engine performs the arithmetic.</small></details>
