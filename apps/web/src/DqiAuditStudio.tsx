...
const examples = [
  ...
  'Show estimated model cost by vendor for confidential data over 12 weeks as ranked horizontal bars',
  'Rank unresolved EU AI Act findings by business unit and severity using sunset columns',
  'Trend hallucination rate by model in production over 26 weeks as a smooth area chart',
  'Show human override rate by user role for high-risk systems as a donut chart',
  'Compare assessment pass rate by regulation in the EU region as horizontal bars',
  'Which integrations have the highest SLA breach rate for restricted data?',
  'Show evidence completeness by DQI system as a light ocean KPI',
  'Trend token cost by model vendor for production over the last 12 weeks'
];
...
export function DqiAuditStudio() {
  ...
  async function refineView(editInstruction: string) {
    setLoading(true); setError('');
    try {
      const response = await fetch('/api/v1/widgets/refine', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ originalPrompt, editPrompt: editInstruction }) });
  ...
  return <main className="dqi-shell">
    <nav className="dqi-nav"><div className="dqi-brand"><span>DQI</span><div><strong>Audit Analytics</strong><small>AI evidence & compliance reporting</small></div></div><div className="nav-actions"><a className="wheel-link" href="/dqi-wheel.html">Explore the DQI 3D Wheel →</a><div className="dqi-demo"><i/>Synthetic audit event store</div></div></nav>
    ...
    <section className="studio-workspace"><aside className="studio-controls"><section className="prompt-panel"><div className="prompt-heading"><label htmlFor="audit-prompt">{result ? 'What would you like to change?' : 'Describe your audit report'}</label><span>{result ? 'Conversational edit' : 'Governed prompt'}</span></div><textarea id="audit-prompt" value={prompt} onChange={(event) => setPrompt(event.target.value)} onKeyDown={(event) => { if ((event.ctrlKey || event.metaKey) && event.key === 'Enter' && !loading && prompt.trim().length >= 3) submitPrompt(); }} placeholder={result ? 'e.g. By policy instead, EU only, make it a trend, or use blue bars…' : 'Ask a question about your DQI audit evidence…'} rows={4} maxLength={1000}/><div className="chat-suggestions">{chatSuggestions.map((item) => <button key={item} onClick={() => setPrompt(item)}>{item}</button>)}</div><div className="prompt-actions"><small>{prompt.length} / 1,000 · Ctrl/Cmd + Enter</small><button onClick={submitPrompt} disabled={loading || prompt.trim().length < (result ? 3 : 10)}>{loading ? 'Updating report…' : result ? 'Apply to report →' : 'Generate compliance widget'}</button></div></section>
...
function AuditWidget({ result }: { result: WidgetGenerationResponse }) {
  ...
  const option = useMemo(() => {
    ...
    if (widget.visual.chartType === 'horizontalBar') return { color: colors, tooltip: { trigger: 'axis' }, grid: { left: 130, right: 24, top: 20, bottom: 35 }, xAxis: { type: 'value', axisLabel: { color: muted }, splitLine: { show: widget.visual.showGrid, lineStyle: { color: grid } } }, yAxis: { type: 'category', data: series.map((item) => item.label), axisLabel: { color: muted }, axisLine: { lineStyle: { color: grid } } }, series: [{ name: widget.metric.label, type: 'bar', barMaxWidth: 26, data: series.map((item) => item.points.at(-1)?.value ?? 0), itemStyle: { borderRadius: [0, 6, 6, 0] } }] };
